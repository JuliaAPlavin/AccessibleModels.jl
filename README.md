...
lines(0..5, x -> mod0(x))
lines!(0..5, x -> mod0.comps[1](x))
lines!(0..5, x -> mod0.comps[2](x))
```
...
true_model = SumFunction((
    ExpFunction(2.0, 3.0),
    ExpFunction(1.5, 7.0),
))
data = [(x=x, y=true_model(x) + 0.2 * randn()) for x in 0:0.5:10]
# data: 21-element Vector{@NamedTuple{x::Float64, y::Float64}}:
#  (x = 0.0, y = 0.158)
#  (x = 0.5, y = -0.172)
#  (x = 1.0, y = -0.138)
#  <...>

loglike(m::SumFunction, data) = sum(r -> logpdf(Normal(m(r.x), 0.3), r.y), data)
...
pt = pigeons(target=amodel, record=[traces; round_trip; record_default()])

samples(pt)
# Retrieves a vector of SumFunction objects with sampled parameters:
# 1024-element Vector{SumFunction}:
...
# Assembles a SumFunction with Particles for parameters
# Each parameter becomes a Particles object with uncertainty estimates
mcmc_fitted = samples(Particles, pt)
# result:
...
```julia
lines(0:0.1:10, x -> mcmc_fitted(x))
band!(0:0.1:10, x -> mcmc_fitted(x))
```
